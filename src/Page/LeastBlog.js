function LeastBlog(){
    return(
        <div className="content__leastblog">
          <h1>Leatest Blog</h1>
          <div className="content__leastblog__leastblogs">
            <div className="content__leastblog__leastblogs__blog">
              <img src="/images/JIUjvqe2ZHg.svg"></img>
              <div className="content__leastblog__leastblogs__blog__top">
                <div className="content__leastblog__leastblogs__blog__top__place">
                  <img src="/images/Vector.svg"></img>
                  <p>SaberAli</p>
                </div>
                <div className="content__leastblog__leastblogs__blog__top__time">
                  <img src="/images/Vector1.svg"></img>
                  <p>21 August,2020</p>
                </div>
              </div>
              <h2>Top esssential Trends in 2021</h2>
              <h4>
                More off this less hello samlande lied much over tightly circa
                horse taped mightly
              </h4>
              <h3>Read More</h3>
            </div>

            <div className="content__leastblog__leastblogs__blog">
              <img src="/images/2dcYhvbHV-M.svg"></img>
              <div className="content__leastblog__leastblogs__blog__top">
                <div className="content__leastblog__leastblogs__blog__top__place">
                  <img src="/images/Vector.svg"></img>
                  <p>SaberAli</p>
                </div>
                <div className="content__leastblog__leastblogs__blog__top__time">
                  <img src="/images/Vector1.svg"></img>
                  <p>21 August,2020</p>
                </div>
              </div>
              <h2>Top esssential Trends in 2021</h2>
              <h4>
                More off this less hello samlande lied much over tightly circa
                horse taped mightly
              </h4>
              <h3>Read More</h3>
            </div>

            <div className="content__leastblog__leastblogs__blog">
              <img src="/images/2dcYhvbHV-M2.svg"></img>
              <div className="content__leastblog__leastblogs__blog__top">
                <div className="content__leastblog__leastblogs__blog__top__place">
                  <img src="/images/Vector.svg"></img>
                  <p>SaberAli</p>
                </div>
                <div className="content__leastblog__leastblogs__blog__top__time">
                  <img src="/images/Vector1.svg"></img>
                  <p>21 August,2020</p>
                </div>
              </div>
              <h2>Top esssential Trends in 2021</h2>
              <h4>
                More off this less hello samlande lied much over tightly circa
                horse taped mightly
              </h4>
              <h3>Read More</h3>
            </div>
          </div>
        </div>
    )
}
export default LeastBlog;